...
class AvlTree {
    ...
    private:
        struct AvlNode {
            AvlNode(T d) : data(d), left(0), right(0), height(0) { }
            T data;
            AvlNode *left, *right;
            unsigned height;
        }; 
        ...
        void insert(AvlNode* &node, T d) {
            ...
            if (d < node->data) {
                ...
                // if ((node->left ? node->left->height : 0) - 
                    // (node->right ? node->right->height : 0) == 2) {
                ...
                // if ((node->right ? node->right->height : 0) - 
                    // (node->left ? node->left->height : 0) == 2) {
